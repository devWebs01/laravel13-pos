...
$totalProfit = computed(function () {
    $items = TransactionItem::whereHas('transaction', function ($q) {
        $q->whereBetween('created_at', [$this->from_date . ' 00:00:00', $this->to_date . ' 23:59:59']);
    })->with('product')->get();

    $revenue = $items->sum('subtotal');
    $cost = $items->sum(fn ($i) => ($i->product?->buy_price ?? 0) * $i->quantity);
    $profit = $revenue - $cost;
...
$lowStockProducts = computed(function () {
    return \App\Models\Product::with('units')
    ...
    @volt
        <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
                    <p class="text-sm text-zinc-500">{{ __('Revenue') }}</p>
                    <p class="mt-1 text-2xl font-bold">{{ Number::currency($this->totalProfit['revenue'], 'IDR', 'id') }}</p>
                </div>
                <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
                    <p class="text-sm text-zinc-500">{{ __('Cost') }}</p>
                    <p class="mt-1 text-2xl font-bold">{{ Number::currency($this->totalProfit['cost'], 'IDR', 'id') }}</p>
                </div>
                <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
                    <p class="text-sm text-zinc-500">{{ __('Profit') }}</p>
                    <p class="mt-1 text-2xl font-bold text-emerald-600">{{ Number::currency($this->totalProfit['profit'], 'IDR', 'id') }}</p>
                </div>
                <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
                    <p class="text-sm text-zinc-500">{{ __('Margin') }}</p>
                    <p class="mt-1 text-2xl font-bold">{{ $this->totalProfit['margin'] }}%</p>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
                <flux:heading size="lg" class="mb-4">{{ __('Low Stock Products') }}</flux:heading>
                <div class="space-y-2">
                    @foreach ($this->lowStockProducts as $product)
                        <div class="flex items-center justify-between rounded-lg border border-zinc-100 p-3 text-sm dark:border-zinc-700">
                            ...
                            <span class="text-xs text-zinc-500">
                                {{ __('Min') }}: {{ $product->min_stock ?: 5 }}
                                &middot;
                                {{ __('Rekomendasi beli') }}: {{ max(0, ($product->min_stock ?: 5) * 2 - $product->stock) }}
                            </span>
                        </div>
                    @endforeach
                </div>
