...
flowchart LR
  ...
  Kasir --- UC01 & UC02 & UC03 & UC04 & UC05 & UC06 & UC07
  Kasir --- UC08 & UC09 & UC10 & UC11 & UC12
  Kasir --- UC13 & UC14 & UC15 & UC16 & UC17
  Kasir --- UC18 & UC19 & UC20
  Kasir --- UC21 & UC22 & UC23

  Admin --- UC01 & UC02 & UC03
  Admin --- UC12
...
|----|-------|-----------|-----------------|
| 1 | Kasir | Pengguna dengan hak akses penuh terhadap seluruh sistem. Kasir bertanggung jawab dalam pengelolaan master data, transaksi penjualan, pengaturan sistem, dan manajemen user | Kelola seluruh modul (master data, transaksi, laporan, user, role, permission, settings) |
| 2 | Admin | Pengguna yang bertugas melakukan monitoring dan analisis bisnis. Admin membutuhkan data laporan untuk pengambilan keputusan | Lihat dashboard, generate laporan, lihat riwayat transaksi, export laporan ke PDF/Excel, update profil |
...
|------|----------|-----------|-------|
| UC-01 | Login | Proses autentikasi user ke dalam sistem menggunakan email dan password | Kasir, Admin |
| UC-02 | Setup 2FA | Mengaktifkan atau menonaktifkan fitur Two Factor Authentication | Kasir, Admin |
| UC-03 | Logout | Proses keluar dari sistem dan menghapus session | Kasir, Admin |
| UC-04 | Kelola Kategori | Mengelola data kategori produk (Create, Read, Update, Delete) | Kasir |
| UC-05 | Kelola Produk | Mengelola data produk termasuk informasi detail, harga, dan stok | Kasir |
| UC-06 | Upload Gambar Produk | Mengunggah dan mengelola gambar untuk setiap produk | Kasir |
| UC-07 | Kelola Stok | Melakukan penyesuaian stok produk secara manual | Kasir |
| UC-08 | Transaksi Penjualan | Melakukan proses transaksi penjualan dari awal hingga akhir | Kasir |
| UC-09 | Tambah Item ke Cart | Menambahkan produk ke dalam keranjang belanja | Kasir |
| UC-10 | Proses Pembayaran | Melakukan kalkulasi dan memproses pembayaran transaksi | Kasir |
| UC-11 | Cetak Struk | Mencetak struk atau invoice hasil transaksi | Kasir |
| UC-12 | Lihat Riwayat Transaksi | Melihat daftar transaksi yang telah dilakukan | Kasir, Admin |
| UC-13 | Laporan Penjualan | Menghasilkan laporan penjualan berdasarkan periode waktu | Kasir, Admin |
| UC-14 | Laporan Stok | Menghasilkan laporan stok produk termasuk stok menipis | Kasir, Admin |
| UC-15 | Laporan Produk Terlaris | Menghasilkan laporan produk dengan penjualan tertinggi | Kasir, Admin |
| UC-16 | Export PDF | Mengekspor laporan ke dalam format PDF | Kasir, Admin |
| UC-17 | Export Excel | Mengekspor laporan ke dalam format Excel | Kasir, Admin |
| UC-18 | Kelola User | Mengelola data pengguna sistem (CRUD) | Kasir |
| UC-19 | Kelola Role | Mengelola role atau peran pengguna dalam sistem | Kasir |
| UC-20 | Kelola Permission | Mengelola hak akses untuk setiap role | Kasir |
| UC-21 | Settings Toko | Mengatur konfigurasi toko (nama, alamat, logo, telepon) | Kasir |
| UC-22 | Settings Profil | Memperbarui profil pengguna (nama, email) | Kasir, Admin |
| UC-23 | Ubah Password | Mengganti password akun pengguna | Kasir, Admin |
...
#### Deskripsi Use Case Kasir dan Admin

Deskripsi use case yang dapat dilakukan oleh aktor Kasir dan Admin adalah sebagai berikut.
...
Activity diagram untuk aktor Kasir mencakup seluruh fungsionalitas sistem mulai dari autentikasi, pengelolaan master data, transaksi, laporan, hingga pengaturan sistem.
...
##### Gambar 4.3 Activity Diagram Login Kasir

```mermaid
flowchart TD
  start((Mulai))
  A_A["Kasir: Buka halaman login"]
  start --> A_A
  S_A["Sistem: Tampilkan form login"]
  A_A --> S_A
  A_B["Kasir: Input email dan password"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Login"]
  A_B --> A_C
  ...
  S_E --> A_D["Kasir: Input kode OTP dari authenticator"]
  A_D --> A_E["Kasir: Submit OTP"]
  ...
  S_G --> A_F["Kasir: Masuk ke halaman dashboard"]
  A_F --> end_success((Selesai))
```

*Sumber: Hasil Perancangan (2026)*

Pada gambar 4.3 activity diagram login, proses yang dilakukan aktor kasir yaitu membuka halaman login, mengisi email dan password, kemudian klik login. Sistem akan memvalidasi format input dan mencocokkan kredensial dengan database. Jika kredensial tidak cocok maka sistem menampilkan pesan error dan memberikan kesempatan untuk ulang login. Jika kredensial valid dan 2FA aktif, kasir harus memasukkan kode OTP dari aplikasi authenticator. Jika OTP valid maka sistem membuat session dan mengarahkan ke halaman dashboard.
#### 4.2.3.2 Logout
...
##### Gambar 4.4 Activity Diagram Logout Kasir

```mermaid
flowchart TD
  start((Mulai))
  A_A["Kasir: Klik menu Logout"]
  start --> A_A
...
Pada gambar 4.4 activity diagram logout, proses yang dilakukan aktor kasir yaitu memilih menu logout. Sistem menampilkan dialog konfirmasi logout. Jika Kasir memilih tidak maka proses dibatalkan. Jika Kasir memilih ya maka sistem menghapus session user dan mengarahkan kembali ke halaman login.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pilih menu Kategori"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman daftar kategori"]
  A_A --> S_A
  A_B["Kasir: Klik tombol Tambah"]
  S_A --> A_B
  S_B["Sistem: Tampilkan form tambah kategori"]
  A_B --> S_B
  A_C["Kasir: Isi nama kategori"]
  S_B --> A_C
  A_D["Kasir: Klik tombol Simpan"]
  A_C --> A_D
...
Pada gambar 4.5 activity diagram tambah kategori, aktor Kasir memilih menu kategori untuk menampilkan halaman daftar kategori. Kemudian Kasir memilih tombol tambah, sistem menampilkan form input kategori. Kasir mengisi nama kategori dan memilih tombol simpan. Sistem melakukan validasi input, jika data tidak valid maka sistem menampilkan error validasi dan meminta ulang input. Jika data valid maka sistem menyimpan kategori ke dalam database dan menampilkan notifikasi sukses.
...
flowchart TD
  ...
  S_A["Sistem: Tampilkan halaman daftar kategori"]
  A_A --> S_A
  A_B["Kasir: Pilih kategori yang akan diedit"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Edit"]
  A_B --> A_C
  S_B["Sistem: Tampilkan form edit dengan data"]
  A_C --> S_B
  A_D["Kasir: Ubah nama kategori"]
  S_B --> A_D
  A_E["Kasir: Klik tombol Simpan"]
  A_D --> A_E
  S_C["Sistem: Validasi input"]
  A_E --> S_C
  D_A{{"Data valid?"}}
...
Pada gambar 4.6 activity diagram edit kategori, aktor Kasir memilih menu kategori untuk menampilkan halaman daftar kategori. Kemudian Kasir memilih kategori yang akan diedit dan memilih tombol edit. Sistem menampilkan form edit dengan data yang sudah ada. Kasir mengubah nama kategori dan memilih tombol simpan. Sistem melakukan validasi input, jika tidak valid maka menampilkan error. Jika valid maka sistem memperbarui data kategori di database dan menampilkan notifikasi sukses.
...
flowchart TD
  ...
  S_A["Sistem: Tampilkan halaman daftar kategori"]
  A_A --> S_A
  A_B["Kasir: Pilih kategori yang akan dihapus"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Hapus"]
  A_B --> A_C
...
Pada gambar 4.7 activity diagram hapus kategori, aktor Kasir memilih menu kategori untuk menampilkan halaman daftar kategori. Kemudian Kasir memilih kategori yang akan dihapus dan memilih tombol hapus. Sistem menampilkan dialog konfirmasi hapus. Jika Kasir memilih tidak maka proses dibatalkan. Jika Kasir memilih ya maka sistem menghapus kategori dari database dan menampilkan notifikasi sukses.
...
flowchart TD
  ...
  A_B --> S_B
  A_C["Kasir: Isi nama, SKU, harga, stok, kategori"]
  S_B --> A_C
  A_D["Kasir: Upload gambar produk (opsional)"]
  A_C --> A_D
  A_E["Kasir: Klik tombol Simpan"]
  A_D --> A_E
  S_C["Sistem: Validasi input"]
...
Pada gambar 4.8 activity diagram tambah produk, aktor Kasir memilih menu produk untuk menampilkan halaman daftar produk. Kemudian Kasir memilih tombol tambah, sistem menampilkan form input produk. Kasir mengisi data produk meliputi nama, SKU, harga, stok, dan kategori serta dapat mengunggah gambar produk secara opsional, kemudian memilih tombol simpan. Sistem melakukan validasi input, jika data tidak valid maka menampilkan error. Jika valid maka sistem menyimpan data produk dan gambar ke dalam database serta menampilkan notifikasi sukses.
#### 4.2.3.7 Edit Produk
...
flowchart TD
  ...
  S_A["Sistem: Tampilkan halaman daftar produk"]
  A_A --> S_A
  A_B["Kasir: Pilih produk yang akan diedit"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Edit"]
  A_B --> A_C
  S_B["Sistem: Tampilkan form edit dengan data"]
  A_C --> S_B
  A_D["Kasir: Ubah data produk"]
  S_B --> A_D
  A_E["Kasir: Ganti gambar (jika perlu)"]
  A_D --> A_E
  A_F["Kasir: Klik tombol Simpan"]
  A_E --> A_F
  S_C["Sistem: Validasi input"]
  A_F --> S_C
  D_A{{"Data valid?"}}
...
Pada gambar 4.9 activity diagram edit produk, aktor Kasir memilih menu produk untuk menampilkan halaman daftar produk. Kemudian Kasir memilih produk yang akan diedit dan memilih tombol edit. Sistem menampilkan form edit dengan data yang sudah ada. Kasir mengubah data produk dan dapat mengganti gambar jika diperlukan, kemudian memilih tombol simpan. Sistem melakukan validasi input, jika tidak valid maka menampilkan error. Jika valid maka sistem memperbarui data produk di database dan menampilkan notifikasi sukses.
...
flowchart TD
  ...
  S_A["Sistem: Tampilkan halaman daftar produk"]
  A_A --> S_A
  A_B["Kasir: Pilih produk yang akan dihapus"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Hapus"]
  A_B --> A_C
  S_B["Sistem: Tampilkan dialog konfirmasi"]
...
Pada gambar 4.10 activity diagram hapus produk, aktor Kasir memilih menu produk untuk menampilkan halaman daftar produk. Kemudian Kasir memilih produk yang akan dihapus dan memilih tombol hapus. Sistem menampilkan dialog konfirmasi hapus. Jika Kasir memilih tidak maka proses dibatalkan. Jika Kasir memilih ya maka sistem menghapus data produk dan gambar dari database serta menampilkan notifikasi sukses.
...
flowchart TD
  ...
  S_F --> D_B{{"Kasir ingin ubah quantity?"}}
  D_B -->|Tidak| S_I
  D_B -->|Ya| A_D["Kasir: Input quantity baru"]
  ...
  D_D -->|Ya| A_B
  D_D -->|Tidak| A_E["Kasir: Input nama customer"]
  A_E --> A_F["Kasir: Pilih metode pembayaran"]
  A_F --> S_J["Sistem: Tampilkan total amount"]
  S_J --> A_G["Kasir: Input jumlah pembayaran"]
  ...
  S_M --> A_H["Kasir: Konfirmasi transaksi"]
  A_H --> A_I["Kasir: Klik Process Payment"]
  ...
  S_U --> A_J["Kasir: Klik Print untuk mencetak struk"]
...
Pada gambar 4.11 activity diagram transaksi penjualan, aktor Kasir membuka halaman transaksi POS, sistem menampilkan interface dengan daftar produk dan menginisialisasi keranjang kosong. Kasir mencari produk berdasarkan nama atau SKU, sistem menyaring dan menampilkan hasil pencarian secara real-time. Kasir memilih produk, sistem memeriksa ketersediaan stok. Jika stok tidak tersedia maka menampilkan notifikasi dan kembali ke pencarian. Jika stok tersedia maka sistem menambahkan produk ke keranjang. Kasir dapat mengubah quantity yang akan divalidasi sistem. Proses diulang untuk setiap produk. Setelah selesai, Kasir memasukkan nama customer dan memilih metode pembayaran. Kasir memasukkan jumlah pembayaran yang divalidasi sistem. Jika pembayaran cukup maka sistem memproses transaksi dengan menyimpan data ke database, mengurangi stok, dan menampilkan struk.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pilih menu Riwayat Transaksi"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman riwayat transaksi"]
  A_A --> S_A
  A_B["Kasir: Filter berdasarkan tanggal (opsional)"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Cari"]
  A_B --> A_C
...
da data?"}}
  ...
  S_C --> A_D["Kasir: Klik transaksi untuk lihat detail"]
...
Pada gambar 4.12 activity diagram lihat riwayat transaksi, aktor Kasir memilih menu riwayat transaksi, sistem menampilkan halaman riwayat transaksi. Kasir memilih kriteria filter seperti tanggal atau status, sistem menampilkan daftar transaksi sesuai kriteria. Kasir memilih salah satu transaksi untuk melihat detail, sistem menampilkan informasi lengkap transaksi tersebut.
...
flowchart TD
  ...
  A_B --> S_B
  A_C["Kasir: Isi nama, email, password, role"]
  S_B --> A_C
  A_D["Kasir: Klik tombol Simpan"]
  A_C --> A_D
  S_C["Sistem: Validasi input"]
...
Pada gambar 4.13 activity diagram tambah user, aktor Kasir memilih menu user untuk menampilkan halaman daftar user. Kemudian Kasir memilih tombol tambah, sistem menampilkan form input user. Kasir mengisi data user berupa nama, email, password, dan role, kemudian memilih tombol simpan. Sistem melakukan validasi input dan memeriksa keunikan email. Jika data tidak valid atau email sudah terdaftar maka sistem menampilkan error. Jika valid maka sistem menyimpan user ke dalam database dan menampilkan notifikasi sukses.
#### 4.2.3.12 Edit User
...
flowchart TD
  ...
  S_A["Sistem: Tampilkan halaman daftar user"]
  A_A --> S_A
  A_B["Kasir: Pilih user yang akan diedit"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Edit"]
  A_B --> A_C
  S_B["Sistem: Tampilkan form edit dengan data"]
  A_C --> S_B
  A_D["Kasir: Ubah data user"]
  S_B --> A_D
  A_E["Kasir: Klik tombol Simpan"]
  A_D --> A_E
  S_C["Sistem: Validasi input"]
...
Pada gambar 4.14 activity diagram edit user, aktor Kasir memilih menu user untuk menampilkan halaman daftar user. Kemudian Kasir memilih user yang akan diedit dan memilih tombol edit. Sistem menampilkan form edit dengan data yang sudah ada. Kasir mengubah data user dan memilih tombol simpan. Sistem melakukan validasi, jika valid maka memperbarui data user di database dan menampilkan notifikasi sukses.
#### 4.2.3.13 Hapus User
...
flowchart TD
  ...
  S_A["Sistem: Tampilkan halaman daftar user"]
  A_A --> S_A
  A_B["Kasir: Pilih user yang akan dihapus"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Hapus"]
  A_B --> A_C
  S_B["Sistem: Tampilkan dialog konfirmasi"]
...
Pada gambar 4.15 activity diagram hapus user, aktor Kasir memilih menu user untuk menampilkan halaman daftar user. Kemudian Kasir memilih user yang akan dihapus dan memilih tombol hapus. Sistem menampilkan dialog konfirmasi hapus. Jika Kasir memilih tidak maka proses dibatalkan. Jika Kasir memilih ya maka sistem menghapus user dari database dan menampilkan notifikasi sukses.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pilih menu Role"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman daftar role"]
  A_A --> S_A
  A_B["Kasir: Pilih role yang akan dikelola"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Edit Permission"]
  A_B --> A_C
  S_B["Sistem: Tampilkan daftar permission"]
  A_C --> S_B
  A_D["Kasir: Centang permission yang diinginkan"]
  S_B --> A_D
  A_E["Kasir: Klik tombol Simpan"]
  A_D --> A_E
...
Pada gambar 4.16 activity diagram kelola role, aktor Kasir memilih menu role untuk menampilkan halaman daftar role. Kemudian Kasir dapat menambah role baru dengan mengisi nama role, mengedit role yang sudah ada, atau menghapus role. Setiap aksi melewati validasi sistem dan konfirmasi sebelum data disimpan atau dihapus dari database.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pilih menu Settings Toko"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman settings"]
  A_A --> S_A
  A_B["Kasir: Ubah nama toko, alamat, telepon, logo"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Simpan"]
  A_B --> A_C
...
Pada gambar 4.17 activity diagram settings toko, aktor Kasir memilih menu settings untuk menampilkan halaman pengaturan toko. Kasir mengedit data toko seperti nama, alamat, telepon, dan logo, kemudian memilih tombol simpan. Sistem memverifikasi data yang diinputkan, jika valid maka menyimpan perubahan ke database dan menampilkan notifikasi sukses.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Klik menu Profil"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman profil"]
  A_A --> S_A
  A_B["Kasir: Ubah nama dan email"]
  S_A --> A_B
  A_C["Kasir: Klik tombol Simpan"]
  A_B --> A_C
  S_B["Sistem: Validasi input"]
...
Pada gambar 4.18 activity diagram update profil, proses yang dilakukan aktor kasir yaitu memilih menu profil, sistem menampilkan halaman profil. Kasir mengubah data profil seperti nama dan email, kemudian memilih tombol simpan. Sistem memvalidasi input, jika data tidak valid maka sistem menampilkan error. Jika valid maka sistem memperbarui data profil di database dan menampilkan notifikasi sukses.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Klik menu Keamanan"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman ubah password"]
  A_A --> S_A
  A_B["Kasir: Input password saat ini"]
  S_A --> A_B
  A_C["Kasir: Input password baru dan konfirmasi"]
  A_B --> A_C
  A_D["Kasir: Klik tombol Simpan"]
  A_C --> A_D
...
Pada gambar 4.18 activity diagram update profil, aktor Kasir memilih menu settings profil untuk menampilkan halaman edit profil. Kasir mengedit data profil berupa nama dan email, kemudian memilih tombol simpan. Sistem memverifikasi data, jika email sudah terdaftar maka menampilkan error. Jika valid maka menyimpan perubahan ke database dan menampilkan notifikasi sukses.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pilih menu Laporan"]
  start --> A_A
  S_A["Sistem: Tampilkan halaman laporan"]
  A_A --> S_A
  A_B["Kasir: Pilih jenis laporan (Penjualan/Stok/Terlaris)"]
  S_A --> A_B
  S_B["Sistem: Tampilkan form filter"]
  A_B --> S_B
  A_C["Kasir: Set periode tanggal"]
  S_B --> A_C
  A_D["Kasir: Pilih filter kategori/produk (opsional)"]
  A_C --> A_D
  A_E["Kasir: Klik Generate Report"]
  A_D --> A_E
  S_C["Sistem: Validasi input"]
  A_E --> S_C
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pada halaman laporan, klik Export PDF"]
  start --> A_A
  ...
  S_E --> S_F
  A_B["Kasir: Simpan file PDF"]
  S_F --> A_B
  A_C["Kasir: Buka dan review file"]
  A_B --> A_C
  end_success([Selesai])
```
...
Pada gambar 4.21 activity diagram export PDF, proses yang dilakukan aktor kasir yaitu memilih tombol export PDF setelah laporan ditampilkan. Sistem mengenerate file PDF dari data laporan dan menyediakan file untuk diunduh.
...
flowchart TD
  start((Mulai))
  A_A["Kasir: Pada halaman laporan, klik Export Excel"]
  start --> A_A
...
Pada gambar 4.22 activity diagram export Excel, aktor Kasir memilih tombol export Excel setelah laporan ditampilkan. Sistem mengenerate file Excel dari data laporan dan menyediakan file untuk diunduh.
---

### Activity Diagram Admin

Activity diagram untuk aktor Admin mencakup fungsionalitas pengelolaan master data (produk dan kategori), monitoring transaksi, serta laporan bisnis.
...
##### Gambar 4.23 Activity Diagram Login Admin
...
Pada gambar 4.23 activity diagram login, proses yang dilakukan aktor admin yaitu membuka halaman login, mengisi email dan password, kemudian klik login. Sistem akan memvalidasi kredensial dengan database. Jika kredensial tidak cocok maka sistem menampilkan pesan error dan memberikan kesempatan untuk ulang login. Jika kredensial valid maka sistem membuat session dan mengarahkan ke halaman dashboard admin.
#### 4.2.3.22 Logout
...
##### Gambar 4.24 Activity Diagram Logout Admin
...
Pada gambar 4.24 activity diagram logout, proses yang dilakukan aktor admin yaitu memilih menu logout. Sistem menampilkan dialog konfirmasi logout. Jika admin memilih tidak maka proses dibatalkan. Jika admin memilih ya maka sistem menghapus session dan mengarahkan kembali ke halaman login.
...
##### Gambar 4.25 Activity Diagram Tambah Kategori (Admin)
...
Pada gambar 4.25 activity diagram tambah kategori, proses yang dilakukan aktor admin yaitu memilih menu kategori untuk menampilkan halaman daftar kategori. Kemudian admin memilih tombol tambah, sistem menampilkan form input kategori. Admin mengisi nama kategori dan memilih tombol simpan. Sistem melakukan validasi input, jika data valid maka menyimpan kategori ke dalam database dan menampilkan notifikasi sukses.
...
##### Gambar 4.26 Activity Diagram Edit Kategori (Admin)
...
Pada gambar 4.26 activity diagram edit kategori, proses yang dilakukan aktor admin yaitu memilih menu kategori untuk menampilkan halaman daftar kategori. Kemudian admin memilih kategori yang akan diedit dan memilih tombol edit. Sistem menampilkan form edit. Admin mengubah nama kategori dan memilih tombol simpan. Sistem memvalidasi dan memperbarui data kategori di database.
...
##### Gambar 4.27 Activity Diagram Hapus Kategori (Admin)
...
Pada gambar 4.27 activity diagram hapus kategori, proses yang dilakukan aktor admin yaitu memilih menu kategori untuk menampilkan halaman daftar kategori. Kemudian admin memilih kategori yang akan dihapus dan memilih tombol hapus. Sistem menampilkan dialog konfirmasi. Jika admin mengonfirmasi maka sistem menghapus kategori dari database dan menampilkan notifikasi sukses.
...
##### Gambar 4.28 Activity Diagram Tambah Produk (Admin)
...
Pada gambar 4.28 activity diagram tambah produk, proses yang dilakukan aktor admin yaitu memilih menu produk untuk menampilkan halaman daftar produk. Kemudian admin memilih tombol tambah, sistem menampilkan form input produk. Admin mengisi data produk dan memilih tombol simpan. Sistem melakukan validasi, jika valid maka menyimpan produk ke database dan menampilkan notifikasi sukses.
...
##### Gambar 4.29 Activity Diagram Edit Produk (Admin)
...
Pada gambar 4.29 activity diagram edit produk, proses yang dilakukan aktor admin yaitu memilih menu produk untuk menampilkan halaman daftar produk. Kemudian admin memilih produk yang akan diedit dan memilih tombol edit. Sistem menampilkan form edit. Admin mengubah data produk dan memilih tombol simpan. Sistem memvalidasi dan memperbarui data produk di database.
...
##### Gambar 4.30 Activity Diagram Hapus Produk (Admin)
...
Pada gambar 4.30 activity diagram hapus produk, proses yang dilakukan aktor admin yaitu memilih menu produk untuk menampilkan halaman daftar produk. Kemudian admin memilih produk yang akan dihapus dan memilih tombol hapus. Sistem menampilkan dialog konfirmasi. Jika admin mengonfirmasi maka sistem menghapus produk dari database dan menampilkan notifikasi sukses.
...
##### Gambar 4.31 Activity Diagram Lihat Riwayat Transaksi (Admin)
...
Pada gambar 4.31 activity diagram lihat riwayat transaksi, proses yang dilakukan aktor admin yaitu memilih menu riwayat transaksi, sistem menampilkan halaman riwayat transaksi. Admin memilih kriteria filter, sistem menampilkan daftar transaksi sesuai kriteria. Admin dapat memilih transaksi untuk melihat detail lebih lanjut.
...
##### Gambar 4.32 Activity Diagram Generate Laporan (Admin)
...
flowchart TD
  ...
  S_B --> A_C
  A_D["Admin: Klik Generate Report"]
  A_C --> A_D
  S_C["Sistem: Validasi input"]
  A_D --> S_C
  D_A{{"Data filter valid?"}}
  ...
  D_B -->|Ya| S_E["Sistem: Format data dan tampilkan tabel"]
  S_E --> S_F["Sistem: Tampilkan grafik dan summary"]
  S_F --> A_E["Admin: Review hasil laporan"]
  A_E --> end_success([Selesai])
```

*Sumber: Hasil Perancangan (2026)*

Pada gambar 4.32 activity diagram generate laporan, proses yang dilakukan aktor admin yaitu memilih menu laporan untuk menampilkan halaman laporan. Admin memilih jenis laporan dan menentukan periode tanggal, kemudian memilih tombol generate. Sistem melakukan query dan menampilkan hasil laporan dalam bentuk tabel dan grafik.
#### 4.2.3.31 Export PDF
...
##### Gambar 4.33 Activity Diagram Export PDF (Admin)
...
Pada gambar 4.33 activity diagram export PDF, proses yang dilakukan aktor admin yaitu memilih tombol export PDF setelah laporan ditampilkan. Sistem mengenerate file PDF dan menyediakannya untuk diunduh.
...
##### Gambar 4.34 Activity Diagram Export Excel (Admin)
...
Pada gambar 4.34 activity diagram export Excel, proses yang dilakukan aktor admin yaitu memilih tombol export Excel setelah laporan ditampilkan. Sistem mengenerate file Excel dan menyediakannya untuk diunduh.
...
##### Gambar 4.35 Activity Diagram Update Profil (Admin)
...
Pada gambar 4.35 activity diagram update profil, proses yang dilakukan aktor admin yaitu memilih menu settings profil untuk menampilkan halaman edit profil. Admin mengedit data profil dan memilih tombol simpan. Sistem memverifikasi data, jika valid maka menyimpan perubahan dan menampilkan notifikasi sukses.
...
##### Gambar 4.36 Activity Diagram Ubah Password (Admin)
...
Pada gambar 4.36 activity diagram ubah password, proses yang dilakukan aktor admin yaitu memilih menu ubah password untuk menampilkan halaman form. Admin mengisi password lama, password baru, dan konfirmasi, kemudian memilih tombol simpan. Sistem memvalidasi password lama dan mencocokkan konfirmasi. Jika valid maka menyimpan password baru ke database.
...
Sequence diagram laporan menggambarkan interaksi yang terjadi ketika user (Admin/Kasir) membuat laporan bisnis. Diagram ini melibatkan objek: User, Browser, Livewire ReportComponent, Transaction Model, Product Model, PDF Generator, Excel Exporter, dan MySQL Database.
...
Relasi ini menggunakan tabel pivot `model_has_roles` dari package Spatie Laravel Permission. Seorang user dapat memiliki lebih dari satu role (misalnya seorang Kasir juga bisa menjadi Admin), dan sebuah role dapat dimiliki oleh banyak user. Relasi ini memungkinkan fleksibilitas dalam pengaturan hak akses.
...
2. **Use Case Diagram** mengidentifikasi dua aktor (Kasir, Admin) dengan 23 use case yang mencakup seluruh fungsionalitas sistem. Setiap use case telah dijelaskan secara detail termasuk aliran utama dan alternatif.

3. **Activity Diagram** memodelkan 34 proses bisnis yang dikelompokkan per aktor (20 diagram Kasir, 14 diagram Admin) meliputi autentikasi, CRUD master data, transaksi, laporan, export, dan pengaturan. Setiap diagram memiliki swimlane aktor dan sistem.
